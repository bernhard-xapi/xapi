...
Both call the
[populate_physmap](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L1395)
hypercall and `xc_domain_populate_physmap_exact()` also sets the flag
for allocating memory only on the given NUMA node.

As an overview, it
[constructs](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L1022)
a `struct memop_args` from the requested reservation
(start address, page size, now many of them, optionally on which NUMA node) and
[passes](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L1458)
it to
[populate_physmap()](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L159) to
[allocate](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L197)
...
The hypercall starts by
[calling](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L1424)
[construct_memop_from_reservation()](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L1022)
to convert the passed parameters from `xen_memory_reservation`
into Xen's `struct memop_args`. It:
...
When a vNUMA vnode is passed using `XENMEMF_vnode`, `domain->vnuma`,
...
[calls](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L1067).
[propagate_node()](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L524)
...
`memory_op()`
[passes](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L1458)
the populated memop to
[populate_physmap()](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L159) to
[loop over the extents](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L197)
to populate:

For each extent in the reservation,
[it calls](https://github.com/xen-project/xen/blob/master/xen/common/memory.c#L275)
[alloc_domheap_pages()](https://github.com/xen-project/xen/blob/master/xen/common/page_alloc.c#L2641)
which
[calls](https://github.com/xen-project/xen/blob/master/xen/common/page_alloc.c#L2673)
[alloc_heap_pages()](https://github.com/xen-project/xen/blob/master/xen/common/page_alloc.c#L968)
which in turn
[calls](https://github.com/xen-project/xen/blob/master/xen/common/page_alloc.c#L1005)
...
[get_free_buddy()](https://github.com/xen-project/xen/blob/master/xen/common/page_alloc.c#L855-L1116)
